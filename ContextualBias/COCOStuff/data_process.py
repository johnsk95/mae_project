...
parser.add_argument('--cocostuff_annotations', type=str, default='Data/cocostuff/annotations')
...
anno_val = sorted(glob.glob('{}/val2017/*.jpg'.format(arg['cocostuff_annotations'])))
anno_train = sorted(glob.glob('{}/train2017/*.jpg'.format(arg['cocostuff_annotations'])))
...
if False:
    ...
    for file in val:
        ...
        for k in temp:
            # print('k: ', k)
            if k < 182:
                labels.append(humanlabels[k+1])
        # label = [humanlabels[k+1] for k in label] # Convert to human-readable labels
        label = [s for s in temp if s not in bad_classes] # Remove bad labels
...
if True:
    ...
    for file in train:
        ...
        # Open the correct things+stuff annotation file
        # print('anno_train_file: ', anno_train_file)
        # print('in? ', anno_train_file in anno)
        # print('anno:', anno)
        if anno_train_file in anno:
            anno_image = Image.open(anno_train_file)
        else:
