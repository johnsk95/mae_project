...
print('171 labels: ', len(humanlabels_171))
...
if False:
    ...
    for file in val:
        ...
        label = list(np.unique(np.array(anno_image)).astype(np.int16)) # len 256
        if 40 in label:
            print('skateboard!')
            anno_image.save(f"skateboard/{file.split('/')[-1]}")
        # print('label: ', label)
        if 255 in label:
            label.remove(255) # Remove class 'unlabeled'
        # if 0 in label:
        #     label.remove(0) # Remove class 'unlabeled'
        # print('label: ', label)
        # print('humanlabel: ', humanlabels)
